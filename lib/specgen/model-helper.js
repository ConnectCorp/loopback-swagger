...
var definitionFunction = function(modelCtor, typeRegistry) {
  ...
  Object.keys(properties).forEach(function(key) {
    ...
    //Make sure numeric ids are always integers
    if (prop.id && prop.type === Number && (!schema.format || schema.format === 'double')) {
      schema.format = 'int64';
    }
  ...
  for (var r in modelCtor.relations) {
    ...
    //This code adds relations to the swagger definition as properties.
    //So these properties can be populated by generated client (via `include` filter)
    var schema = schemaBuilder.buildFromLoopBackType(rel.modelTo || rel.modelThrough, typeRegistry);
    switch (rel.type) {
      case 'hasOne':
      case 'belongsTo':
        swaggerDef.properties[rel.name] = schema;
        break;
      case 'hasMany':
        swaggerDef.properties[rel.name] = {
          type : 'array',
          items: schema
        };
        break;
